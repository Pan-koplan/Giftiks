...
<div class="container_reader">
        ...
        <div class="recent-picks">
          ...
          @foreach ($articles as $article_rec)
            <div class="recent-item">
              <img src="{{ Storage::url($article_rec->image) }}" alt="{{$article_rec->title}}">
    ...
    <div class="content">
      <h1 style="margin-left: 0px;  margin-top: 0px;">{{$article->title}}</h1>
        <img src="{{ Storage::url($article->image) }}" alt="Топ-10 подарков" class="header-img">
        <p>{{$article->intro}}</p>
        @for ($i = 0; $i < count($gifts); $i++)
          <div class="gift-item" id="{{$i}}">
              <img src="{{Storage::url('/Images/' . $gifts[$i]->source . '_' . $gifts[$i]->id . '_1.jpg')}}" alt="{{$gifts[$i]->name}}">
              <div class="gift_item_text_article">
                  ...
                  <div class="buttons_mobile">
                    <a href="<?= $gifts[$i]->link ?>"><button class="button"><svg height="512" viewBox="0 0 512 512" width="512" xmlns="http://www.w3.org/2000/svg"><title/><path d="M456.69,421.39,362.6,327.3a173.81,173.81,0,0,0,34.84-104.58C397.44,126.38,319.06,48,222.72,48S48,126.38,48,222.72s78.38,174.72,174.72,174.72A173.81,173.81,0,0,0,327.3,362.6l94.09,94.09a25,25,0,0,0,35.3-35.3ZM97.92,222.72a124.8,124.8,0,1,1,124.8,124.8A124.95,124.95,0,0,1,97.92,222.72Z" fill="currentColor"/></svg></button></a>
